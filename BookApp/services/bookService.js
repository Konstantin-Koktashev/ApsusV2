...
function createTimeStamp() {
  return `${Date.now().getYear()}   ${Date.now().getHours()}:${Date.now().getMinutes()} `
}
...
function _createBook(thumbnail, title, authors, amount) {
  return {
...
function save(bookToSave) {
  var savedBook = bookToSave;
  if (bookToSave.id) {
      const bookIdx = _getIdxById(bookToSave.id)
      gBooks[bookIdx] = bookToSave;
  } else {
      savedBook = _createBook(bookToSave.thumbnail, bookToSave.title, bookToSave.authors, bookToSave.amount)
      gBooks.push(savedBook)
  }
  storageService.store(STORAGE_KEY, gBooks)
...
function query(filterBy) {
  let books = gBooks;
    if (filterBy) {
        ...
        books = gBooks.filter(book => book.title.includes(name)
...
function setAmountColor(amount) {
